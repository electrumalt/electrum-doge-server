...
class BlockchainProcessor(Processor):
    ...
    def format_auxpow(self, auxpow):
        auxpow['parent_block'] = parent_block_to_header(auxpow['parent_block'])
        return auxpow

    def block2header(self, b):
        # dogecoin todo: add auxpow
        #print b
        res = {
    ...
    def init_headers(self, db_height):
        ...
        self.auxpows_filename = os.path.join(self.headers_path, 'blockchain_auxpows')
            ...
            while height < db_height:
                ...
                if height > 1:
                    if prev_hash != header.get('prev_block_hash'):
                        ...
                        height -= 2
                        print Hash(header_to_string(self.read_header(height))[0:80].decode('hex')).encode('hex')
                        prev_hash = self.hash_header(self.read_header(height))
                        continue
    ...
    def hash_header(self, header):
        # OK dogecoin todo: remove auxpow offsets before hashing
    ...
    def read_chunk(self, index):
        # dogecoin todo: include auxpow data in chunk...
        with open(self.headers_filename, 'rb') as f:
            f.seek(index*2016*88)
            chunk = f.read(2016*88)

        # todo: also read auxpow data
        first_header = header_from_string(chunk[0:88])
        last_header = header_from_string(chunk[-88:])

        auxpow_chunk = ''
        with open(self.auxpows_filename, 'rb') as f:
            start = first_header['auxpow_offset']
            end = last_header['auxpow_offset'] + last_header['auxpow_length']
            length = end - start
            f.seek(start)
            auxpow_chunk = f.read(length)

        print 'read chunk ', index
        # todo: append auxpow data?
        #return chunk.encode('hex')
        #print auxpow_chunk
        #exit()
        num_headers = len(chunk) / 88
        print num_headers, 'headers'
        res = (chunk + auxpow_chunk).encode('hex')
        print 'res without num', res[0:100]
        res = int_to_hex(num_headers, 4) + res
        print 'res with num', res[0:100]
        return res
    ...
    def write_header(self, header, sync=True):
        ...
        if header.get('auxpow'):
            auxpow_str = auxpow_to_string(header.get('auxpow'))
            self.auxpows_data += auxpow_str
            header['auxpow_offset'] = self.auxpows_offset
            header['auxpow_length'] = len(auxpow_str)
            self.auxpows_offset += len(auxpow_str)
        else:
            header['auxpow_offset'] = self.auxpows_offset
            header['auxpow_length'] = 0
        # /auxpow
        ...
        # dogecoin todo: not sure why 40*100 was chosen... half of 80?
    ...
    def flush_headers(self):
        ...
        with open(self.auxpows_filename, 'a+') as f:
            # dogecoin todo: always write auxpow data at end of file?
    ...
    # dogecoin todo: not sure if the 80 here is related to block header
