...
def auxpow_to_string(auxpow):
    s = ''
    s += tx_to_string(auxpow['coinbasetx']) # coinbase tx
    s += merkle_branch_to_string(auxpow['coinbaseMerkleBranch'], auxpow['coinbaseIndex']) # coinbase branch
    s += merkle_branch_to_string(auxpow['chainMerkleBranch'], auxpow['chainIndex']) # blockchain branch
    s += header_to_string(auxpow['parent_block'])
    return s
...
def merkle_branch_to_string(branch, index):
    s = ''
    s += var_int(len(branch)) # branch length
    for _hash in branch:
        s += rev_hex(_hash)   # branch hash
    s += int_to_hex(int(index), 4) # index bitmask
    return s
...
def tx_from_string(s):
    ...
    for i in xrange(n_vout):
        ...
        #print s[start:vds.read_cursor].encode('hex')
        #print txout
        d['vout'].append(txout)
...
def tx_to_string(tx):
    s = ''
    s += int_to_hex(int(tx['version']), 4) # version

    s += var_int(len(tx['vin'])) # txin count

    for txin in tx['vin']:

        if not 'coinbase' in txin:
            raise Error('Can only serialize coinbase tx')

        s += var_int(len(txin['coinbase'].decode('hex')))
        s += txin['coinbase']
        s += int_to_hex(int(txin['sequence']), 4)

    s += var_int(len(tx['vout'])) # txout count

    for txout in tx['vout']:
        s_txout = ''
        #THIS IS NOT RIGHT
        #  s_txout += int_to_hex(int(txout['value'] * COIN), 8) # value
        #BECAUSE
        #  >>> 5.05459139 * 1e8
        #  505459138.99999994
        s_txout += int_to_hex(long(round(txout['value'] * COIN)), 8) # value
        script = txout['scriptPubKey']['hex']
        s_txout += var_int(len(script.decode('hex'))) # script length
        s_txout += script # script
        s += s_txout

    s += int_to_hex(int(tx['locktime']), 4) # locktime
    return s

def parent_block_to_header(parent_block):
    h = {}
...
def header_to_string(res):
    ...
    #OK ixcoin todo: calculate+include auxpow position in auxpow file (offset(s))
    ...
    if ('auxpow_offset' in res and 'auxpow_length' in res):
        s += int_to_hex(int(res.get('auxpow_offset')), 4)
        s += int_to_hex(int(res.get('auxpow_length')), 4)
...
logger = logging.getLogger('electrum-doge')
